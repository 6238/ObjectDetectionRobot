...
public class ObjectDetectionIOJetson implements ObjectDetectionIO {
    ...
    public ObjectDetectionIOJetson() {
        ...
        heartbeat = jetsonTable.getIntegerTopic("heartbeat")
        ...
        targetConfidences = jetsonTable.getDoubleArrayTopic("targetConfidence").subscribe(new double[0]);
    ...
    public void updateInputs(ObjectDetectionIOInputs inputs) {
        ...
        for (int i = 0; i < currentTargetDXs.length; i++) {
            inputs.targetObservations[i] = new TargetObservation(
                currentTargetDXs[i],
                currentTargetDYs[i],
