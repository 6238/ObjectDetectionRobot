...
// Compute approximate ground-plane position given pixel bottom center
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        std::cout << "NetworkTables initialized for team 6238 (Struct mode)\n";
        ...
        while (true) {
            ...
            for (size_t i = 0; i < boxes.size(); i++) {
                ...
                TargetDetection obs;
                obs.dx = bottom_center_x / static_cast<double>(orig_w);  // Normalized X
                obs.dy = bottom_y / static_cast<double>(orig_h);         // Normalized Y
                obs.area = (box_w * box_h) / static_cast<double>(orig_w * orig_h);  // Normalized area
